...
export default function EditAlbumModal({ isOpen, onClose, onSuccess, albumId }) {
  ...
  const loadAlbumData = async () => {
    ...
    try {
      ...
      setAlbumLocation(album.album_location || "");
      setAlbumCity(album.album_city || "");
      setAlbumState(album.album_state || "");
      setAlbumCountry(album.album_country || "");
      
      // Determine location type based on existing data
      if (album.album_location) {
        setLocationType("straight");
      } else if (album.album_city || album.album_state || album.album_country) {
        setLocationType("separate");
      } else {
        setLocationType("straight"); // Default
      }
    } catch (err) {
      console.error("Error loading album:", err);
      setError(err.response?.data?.detail || "Failed to load album data");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        >
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-8">
              <div className="w-8 h-8 border-4 border-purple-600/30 dark:border-cyan-400/30 border-t-purple-600 dark:border-t-cyan-400 rounded-full animate-spin" />
            </div>
          )}
