...
def delete_raw_image(raw_image_id: int, db: Session = Depends(get_db)):
    ...
    # 3. Delete original file from cold storage (storagePath/raw/)
    ...
    # 4. Delete hot storage files (thumbnails and previews)
...
def get_raw_thumbnail_file(raw_image_id: int, db: Session = Depends(get_db)):
    """
    Serve a JPEG thumbnail of the RAW image.
    The processor should have generated a thumbnail during processing.
    If thumbnail doesn't exist, return 404 (don't fallback to RAW as browsers can't display it).
    """
    raw = db.query(models.RawImage).filter(models.RawImage.id == raw_image_id).first()
    ...
    # Construct expected preview path (e.g., preview_hash.jpg)
    path_hash = hashlib.md5(os.path.join(config.value, 'raw', raw.filename).encode('utf-8')).hexdigest()
    thumbnail_filename = f"thumb_{path_hash}.jpg"
    thumbnail_path = os.path.join(settings.RAW_THUMBNAIL_DIR, thumbnail_filename)
